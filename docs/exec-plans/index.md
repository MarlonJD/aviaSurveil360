...
| [Department And General Manager Oversight Workspaces](active/2026-07-09-department-manager-workspaces-plan.md) | active | Tasks 1-2 are implemented/reviewed; Task 3 Inspection Team is implemented with its final review still pending. Scope now also includes Reports/PDF, restricted role navigation, manager dashboard, CAP Monitoring, Checklist Management, manager Risk Dashboard, and General Manager oversight. **Next:** isolate the existing Task 1 foundation diff, run its focused checks, then commit and push it before continuing. |
