...
**Status:** `ready-for-verification` — the explicitly authorized mock-data first executable slice (Tasks 2-4) is implemented and `verified locally` as `candidate-only`. The canonical product docs and intact verified legacy demo remain the slice-local vocabulary, role-entry, and behavior authority. The 2026-07-20 adversarial review's broader contract, sync, storage, build, migration, and verification corrections remain incorporated, but every later-slice owner decision and Tasks 5-13 remain `blocked` pending separate authorization. Real HTTP, Go, offline, release, and production evidence are `not run`.
...
- [ ] **Step 2: Record named owners and accepted values in the Decision Log.**
...
- [ ] **Step 1: Confirm the approved route classification before each increment.**
...
- [ ] **Step 2: Write failing behavior-ledger, OpenAPI, backend-contract, and browser assertions for one route family.**
...
- [ ] **Step 3: Extend contract, mock, Go, and React together.**
...
- [ ] **Step 4: Verify all visible behavior.**
...
- [ ] **Step 5: Run the accepted React route/role matrix at desktop, tablet, and mobile in both profiles.**
...
- [ ] **Step 6: Keep the legacy demo and accepted-differences ledger available.**
...
- [ ] **Step 1: Write failing tests for atomic response/outbox behavior.**
...
- [ ] **Step 2: Define the IndexedDB schema.**
...
- [ ] **Step 3: Implement write-through-local field behavior.**
...
- [ ] **Step 4: Add migration and failure recovery tests.**
...
- [ ] **Step 5: Run offline storage verification.**
...
- [ ] **Step 1: Write failing staging/recovery tests.**
...
- [ ] **Step 2: Implement staged writes in a dedicated Worker.**
...
- [ ] **Step 4: Verify attachment survival and cleanup.**
...
- [ ] **Step 2: Implement server-side idempotency and revision checks.**
...
- [ ] **Step 3: Implement causal local delivery and one sync owner.**
...
- [ ] **Step 4: Implement authorized pull and atomic cursor application.**
...
- [ ] **Step 5: Implement client sync triggers and backoff.**
...
- [ ] **Step 6: Implement explicit conflict UI.**
...
- [ ] **Step 7: Verify foreground-only fallback and recovery.**
...
| Tasks 2-4 mock-data first executable slice | Current user / plan owner | accepted | Explicit authorization dated 2026-07-20. Tasks 2-4 are implemented and `verified locally`; evidence is [React Mock Slice](../../demo-evidence/REACT_MOCK_SLICE_2026-07-20.md). Result remains `candidate-only`. Tasks 5-13 and every production/offline/Go capability remain unauthorized and `blocked`. |
| Canonical vocabulary and lifecycle mapping | Product + CAA Operations | accepted | Slice-local acceptance by the current user / plan owner on 2026-07-20: use the canonical product docs and verified legacy demo as the authority for Tasks 2-4. This is not a later-slice regulatory, records, or production-owner acceptance. |
| `first-production` / `later` / `demo-only` route inventory | Product | accepted | Slice-local acceptance by the current user / plan owner on 2026-07-20: migrate only the complete canonical Cabin scenario and these eight legacy role entries: Inspector `inspector-assignments`, Lead Inspector `lead-review`, Department Manager `dashboard`, General Manager `gm-dashboard`, Finance Review `finance-review`, Executive Director `executive-dashboard`, Auditee `service-provider-cap`, and Admin `templates`. Every other route remains `later` or `demo-only` in the intact legacy demo; Task 5 classification remains blocked. |
...
| React/Vite and one-module Go ownership ADR | Engineering + Platform | blocked | React/Vite is accepted only for the Tasks 2-4 candidate slice by the current user / plan owner. Go implementation, deployment, observability, and on-call ownership remain blocked before Tasks 9-11. |
| Same-origin OIDC session/BFF, MFA, cookie, CSRF, expiry, and revocation | Security + Identity | blocked | Must be accepted before Task 9-10 auth implementation. |
| Managed browser/device/profile and local-data protection | Security + CAA Operations + Records | blocked | Must be accepted before Task 6 official offline readiness. |
| Offline grant/package duration, scope, reassignment/revoke/late-sync behavior | Product + Security + CAA Operations | blocked | Must be accepted before Tasks 6-7 and server checkout implementation. |
| Inspection Attachment versus official Evidence; size/media/checksum/upload/scan | Product + Records + Security | blocked | Must be accepted before Tasks 8 and 11. First protocol defaults to bounded whole-object retry, not multipart/resume. |
| Conflict policy | Product + CAA Operations | blocked | First protocol defaults to no automatic merge; any exception requires explicit approval and tests. |
| Retention/legal hold/disposition and audit tamper-evidence claim | Records + Legal + Security | blocked | Destructive retention stays out of scope; append-only is the only audit-storage claim until accepted. |
| Hosting region, PostgreSQL/object storage/scanner, RPO/RTO, restore/DR, monitoring | Platform + Operations | blocked | Local integration may use isolated test dependencies; production release remains blocked. |
...
- Current status: `ready-for-verification` for the explicitly authorized Tasks 2-4 slice; the broader umbrella objective remains incomplete.
...
- Current next todo: stakeholder/user review of the Tasks 2-4 `candidate-only` evidence. Stop after this review handoff; any Tasks 5-13 work requires separate user authorization, and each dependent owner decision remains `blocked`.
