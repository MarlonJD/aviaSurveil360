...
function downloadDocument(
  ...
  if (buildProfile === "http") {
    ...
    onDone(`${fileName} downloaded as the authorized immutable generated Document ${document.documentVersionId ?? document.id}.`);
    return;
  }
...
export function AuditeeDocumentsPage() {
  ...
  async function openAndDownload(document: DocumentMetadataView) {
    ...
    try {
      ...
      downloadDocument(exact, runtime.buildProfile, setStatus);
      setError(null);
  ...
  return <WorkspaceShell roleLabel="Auditee — Fly Namibia" routeLabel="Documents">
    <div className="auditee-secondary-page auditee-documents-page" data-testid="auditee-documents-page">
      ...
      <p className="auditee-safe-boundary">{runtime.buildProfile === "http" ? "Private generated PDFs are downloaded only through short-lived authorized URLs. Rendering does not approve, sign, close, or confer legal validity." : "Filename-only browser downloads; no real storage, signature, or official delivery."}</p>
