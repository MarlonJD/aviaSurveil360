...
assignmentState.role = 'leadInspector';
...
assert.match(preliminaryHtml, /data-act="preliminary-report-status"/);
assert.doesNotMatch(preliminaryHtml, /data-act="preliminary-report-status"[^>]*disabled/);
context.handleAction('preliminary-report-status', dataEl({}));
assert.match(elements.get('modal-host').innerHTML, /Preliminary Report Submission Status/);
assert.match(elements.get('modal-host').innerHTML, /Pending Department Manager Review/);
assert.match(elements.get('modal-host').innerHTML, /Current owner/);
...
assert.match(capTrackingHtml, /CAB-2026-011/);
assert.doesNotMatch(capTrackingHtml, /F-014-|F-2026-/);
...
context.handleAction('cap-track-row-action', dataEl({ 'data-id': 'CAB-2026-011', 'data-track-action': 'view' }));
...
assert.match(capReviewDetailHtml, /CAP-2026-011/);
assert.match(capReviewDetailHtml, /Finding Information/);
assert.match(capReviewDetailHtml, /Emergency equipment serviceability record incomplete/);
assert.match(capReviewDetailHtml, /Corrective Action Plan \(CAP\) by Service Provider/);
assert.match(capReviewDetailHtml, /CAP attachments and Evidence/);
assert.match(capReviewDetailHtml, /Inspector Assessment/);
assert.match(capReviewDetailHtml, /Caner Yildiz/);
assert.match(capReviewDetailHtml, /Next action/);
...
context.handleAction('cap-lead-return', dataEl({ 'data-id': 'CAB-2026-011' }));
assert.equal(context.state.notifications[0].role, 'inspector');
assert.match(context.state.notifications[0].text, /returned CAB-2026-011/);
assert.equal(context.findingById('CAB-2026-011').leadReview.decision, 'returned_to_inspector');

context.handleAction('cap-lead-submit', dataEl({ 'data-id': 'CAB-2026-011' }));
...
assert.equal(context.findingById('CAB-2026-011').leadReview.decision, 'recommend_closure');

context.state.role = 'manager';
context.state.view = 'unit-manager-review';
context.state.params = { findingId: 'CAB-2026-012' };
context.render();
const departmentManagerHtml = elements.get('app-root').innerHTML;
assert.match(departmentManagerHtml, /Department Manager Review - Finding CAB-2026-012/);
...
assert.match(departmentManagerHtml, /Recommend Closure/);
assert.match(departmentManagerHtml, /Request More Information/);
assert.match(departmentManagerHtml, /Record Management Review/);
assert.doesNotMatch(departmentManagerHtml, /Submit Recommendation to General Manager/);

context.handleAction('cap-unit-choose-file', dataEl({ 'data-id': 'CAB-2026-012' }));
assert.match(context.state.capTrackingUi.unitAttachmentName, /department_manager/);
context.handleAction('cap-unit-submit-general-manager', dataEl({ 'data-id': 'CAB-2026-012' }));
...
assert.match(context.state.notifications[0].text, /Department Manager recorded/);
assert.equal(context.findingById('CAB-2026-012').status, 'EVIDENCE_REQUIRED');
assert.equal(context.findingById('CAB-2026-012').closedDate, null);
